...
export default () => {
  ...
  const toNav = () => {
    window.open(`https://www.code-nav.cn/resources?q=${searchText}`);
  }
  const toBaidu = () => {
    window.open(`https://www.baidu.com/s?wd=${searchText}`);
  }
  const toHome = () => {
    window.open(WEB_HOST,'_self');
  }
  ...
  return (
    <div>
      <Layout>
        <Header style={{zIndex: 10}}>
          <Row align="middle" justify="center">
            <Col xl={1} lg={2} md={2} sm={3} xs={4}>
              <div onClick={toHome} style={{cursor: 'pointer'}}><Avatar shape="square" src={logo} /></div>
            </Col>
            ...
            <Col xl={16} lg={14} md={12} sm={10} xs={24}>
              <Menu theme="dark" mode="horizontal" selectedKeys={[current]} onClick={handleClick}
                ...
                <Menu.Item key="code"><IconFont type="icon-code" />编程</Menu.Item>
                <Menu.Item key="baidu" onClick={toBaidu}><IconFont type="icon-baidu" />百度</Menu.Item>
              </Menu>
            </Col>
          </Row>
        </Header>
        ...
        {
          current === "code" &&
          <iframe src={`https://www.code-nav.cn/resources?q=${searchText}`}
                  style={{border: 'none', width: '100%', height: '100vh', position: 'fixed', top: 60, zIndex: 1}}
          />
        }
      </Layout>
    </div>
