...
import multisearch from '../assets/multisearch.png';
import baidu from '../assets/baidu.png';
import google from '../assets/google.png';
import defaultHome from "../constant/defaultHome";
import defaultTabPane, {DEFAULT_COVER1} from "../constant/defaultTabPane";
import {CURRENT_COVER, Bing_Pic, GHOST_OPEN, PROJECT_GITHUB, SEARCH_ALL_OPEN, WEB_HOST} from "../constant";
...
export default () => {
  ...
  // 当前封面
  const currentCover = localStorage.getItem(CURRENT_COVER) ? JSON.parse(localStorage.getItem(CURRENT_COVER)) : DEFAULT_COVER1;
  ...
  const drawerTitle = (
    <div className="drawer-title">
      <a href={WEB_HOST} target="_blank">
        <Avatar src={multisearch} shape="square" style={{marginRight: 12}} />
        <span className="site-title">MyHomePage</span>
      </a>
    </div>
  )
    ...
    <div className={`index${ghostClose ? '' : ' ghost'}`}>
        ...
        <Search enterButton={<Button type={ghostClose ? 'primary' : 'ghost'}><SearchOutlined /></Button>} size="large"
                ...
                suffix={
                  <Tooltip title={`${searchAll ? '关闭' : '开启'}万能搜索`} placement="left">
                    <img className={`search-all-switch${searchAll ? '' : ' close'}`} src={multisearch} width={28} alt="万能搜索"
      ...
      <div className={'mask' + (maskOpened ? '' : ' hidden')} />
      {
        cover.type === 'iframe' ?
          ...
          <img className="bg" src={Bing_Pic} alt="bg" />
      }
    </div>
  )
}
